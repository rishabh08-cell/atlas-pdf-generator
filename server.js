...
function getAtlasTabUrls(baseUrl) {
  ...
  return { overview: root+"/overview", competitors: root+"/competitors-comparison", platforms: root+"/platforms", prompts: root+"/prompts-themes" };
}
...
async function extractData({ screenshots, pageTexts }) {
  ...
  for (let attempt=0; attempt<=MAX_RETRIES; attempt++) {
    if (attempt>0) { const d=RETRY_DELAYS[attempt-1]; console.log(`  ⏳ retrying in ${d/1000}s...`); await new Promise(r=>setTimeout(r,d)); }
    let res;
    try {
...
{ "brandName":"NESTLE","domain":"nestleprofessional.in","totalMentions":28,"totalCitations":33,"avgBrandCoverage":"9.3%","avgDomainCoverage":"9.0%",
  "leaderboard":[{"rank":1,"name":"WMF","mentions":33},{"rank":2,"name":"NESTLE","mentions":28},{"rank":3,"name":"Jura","mentions":21}],
  "brandLeaderboardRank":2,
  "competitorMentions":[{"name":"WMF","percentage":11,"mentions":33},{"name":"NESTLE","percentage":9,"mentions":28}],
  "platforms":[{"name":"ChatGPT","mentions":10,"citations":5,"brandVisibility":3,"domainCoverage":1},{"name":"Google AI Overview","mentions":12,"citations":21,"brandVisibility":4,"domainCoverage":6},{"name":"Perplexity","mentions":6,"citations":7,"brandVisibility":2,"domainCoverage":2}],
  "promptThemes":[{"theme":"24/7 Operations Efficiency & Compliance Management","promptCount":7,"prompts":["best grab and go beverage solutions"]},{"theme":"Beverage Quality Consistency & Menu Diversification","promptCount":8,"prompts":["80 beverage options single machine"]}],
  "competitorVisibilityMatrix":[{"theme":"24/7 Operations Efficiency & Compliance Management","brandVisibility":7,"competitors":{"WMF":0,"Jura":0,"Franke":3}}],
  "brandVisibilityByPlatform":[{"theme":"24/7 Operations Efficiency & Compliance Management","ChatGPT":10,"Google AI Overview":10,"Perplexity":0}],
  "domainCitations":[{"domain":"www.reddit.com","domainCoverage":"14%","uniquePagesCited":72,"domainShare":"3%"}],
  "brandPages":[{"name":"Page title or URL","prompts":5}]
}
Rules: Extract ALL rows. brandLeaderboardRank: read rank directly (e.g. "#5 Netskope" = 5). competitorVisibilityMatrix: ALL rows and ALL competitor columns. brandVisibilityByPlatform: ALL rows. Return ONLY the JSON.` }
          ]}],
        }),
      });
    } catch(networkErr) { lastErr=networkErr; console.warn("  ⚠️ network error:", networkErr.message); continue; }
    if (res.status===529||res.status===503) { const b=await res.text(); lastErr=new Error(`Claude ${res.status}`); console.warn("  ⚠️ Claude",res.status); continue; }
    if (!res.ok) throw new Error(`Claude API error: ${res.status} — ${await res.text()}`);
    const json=await res.json(); const text=json.content[0].text.trim();
    try { data=JSON.parse(text); } catch { const m=text.match(/\{[\s\S]*\}/); if(m) data=JSON.parse(m[0]); else throw new Error("Cannot parse Claude JSON"); }
    lastErr=null; break;
  }
...
function normalizeData(raw) {
  const tm=raw.totalMentions||0, tc=raw.totalCitations||0;
  ...
  const competitorMentions=raw.competitorMentions?.length>0?raw.competitorMentions:leaderboard.map((b,i)=>({name:b.name,percentage:Math.max(30-i*8,2),mentions:b.mentions}));
  const isLeader=leaderboard[0]?.name===raw.brandName;
  const leaderboardRank=raw.brandLeaderboardRank?"#"+raw.brandLeaderboardRank:isLeader?"#1":"#"+(leaderboard.findIndex(b=>b.name===raw.brandName)+1||leaderboard.length);
  return {
    brandName:raw.brandName||"Brand", domain:raw.domain||raw.brandName?.toLowerCase().replace(/\s+/g,"")+".com",
    totalMentions:tm, totalCitations:tc, avgBrandCoverage:raw.avgBrandCoverage||"0%", avgDomainCoverage:raw.avgDomainCoverage||"0%",
    leaderboardRank, platformCount:platforms.length, leaderboard:leaderboard.slice(0,3),
...
function logoPill(s, pres) {
  ...
  s.addShape(pres.shapes.ROUNDED_RECTANGLE, { x:9.05,y:0.06,w:0.80,h:0.20, fill:{color:C.white}, line:{color:C.lightgray,pt:1}, rectRadius:0.06, shadow:makeShadow() });
  if (fs.existsSync(pepperLogoPath)) {
    s.addImage({ path:pepperLogoPath, x:9.08,y:0.08,w:0.70,h:0.14, sizing:{type:"contain",w:0.70,h:0.14} });
  } else {
    s.addText("pepper", { x:9.08,y:0.08,w:0.70,h:0.14, fontSize:9,bold:true,color:C.navy,align:"center",valign:"middle",fontFace:"Calibri" });
  }
}
...
function buildSlide1(pres, d) {
  ...
  const pepperLogoPath = path.join(__dirname, "public", "logos", "pepper-logo.png");
  s.addShape(pres.shapes.ROUNDED_RECTANGLE,{x:9.1,y:0.3,w:0.70,h:0.18,fill:{color:"FFFFFF"},line:{color:"FFFFFF"},rectRadius:0.08});
  if (fs.existsSync(pepperLogoPath)) {
    s.addImage({path:pepperLogoPath,x:9.13,y:0.32,w:0.65,h:0.14,sizing:{type:"contain",w:0.65,h:0.14}});
  } else {
    s.addText("pepper",{x:9.13,y:0.32,w:0.65,h:0.14,fontSize:9,bold:true,color:C.navy,align:"center",valign:"middle",fontFace:"Calibri"});
  }
...
function buildSlide5(pres, d) {
  ...
  matrix.slice(0,11).forEach((row,ri)=>{
    ...
    allCols.forEach((col,ci)=>{
      ...
      let cc=bg; if(isB&&pct>0)cc=C.purple; else if(pct>=15)cc="8B85D4"; else if(pct>=5)cc="C4C0EA";
...
app.post("/generate", async (req, res) => {
  const { url }=req.body;
  if (!url||!url.includes("atlas.pepper.inc")) return res.status(400).json({ error:"Please provide a valid Atlas report URL." });
